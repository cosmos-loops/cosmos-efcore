...
namespace Cosmos.Dependency
{
    ...
    public static class CosmosSqlServerDbContextConfigExtensions
    {
        ...
        public static IDbContextConfig UseEfCoreWithSqlServer<TContext>(this DbContextConfig<MicrosoftProxyRegister> context, Action<EfCoreOptions> optAct = null)
            where TContext : DbContext, IEfContext
        {
            ...
            context.RegisterDbContext(register =>
            {
                if (!string.IsNullOrWhiteSpace(opt.ConnectionString))
                {
                    ...
                    register.RawServices.AddDbContext<TContext>(builder => builder.UseSqlServer(opt.ConnectionString));
                }
                else if (!string.IsNullOrWhiteSpace(opt.ConnectionName))
                {
                    EfCoreOptionsRegistrar.Register<TContext>(opt);
                    register.RawServices.AddDbContext<TContext>((provider, builder) => builder.UseSqlServer(provider.GetService<IConfigurationRoot>().GetConnectionString(opt.ConnectionName)));
                }
                else
                {
