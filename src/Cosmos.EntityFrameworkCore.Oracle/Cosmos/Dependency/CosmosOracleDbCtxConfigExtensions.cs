            ...
            where TContext : DbContext, IEfContext
        {
            ...
            context.RegisterDbContext(register =>
            {
                if (!string.IsNullOrWhiteSpace(opt.ConnectionString))
                {
                    ...
                    register.RawServices.AddDbContext<TContext>(builder => builder.UseOracle(opt.ConnectionString));
                }
                else if (!string.IsNullOrWhiteSpace(opt.ConnectionName))
                {
                    EfCoreOptionsRegistrar.Register<TContext>(opt);
                    register.RawServices.AddDbContext<TContext>((provider, builder) => builder.UseOracle(provider.GetService<IConfigurationRoot>().GetConnectionString(opt.ConnectionName)));
                }
                else
                {
