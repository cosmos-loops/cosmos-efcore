            ...
            where TContext : DbContext, IEfContext
        {
            ...
            context.RegisterDbContext(services =>
            {
                if (!string.IsNullOrWhiteSpace(opt.ConnectionString))
                {
                    ...
                    services.AddDbContext<TContext>(builder => builder.UseMySQL(opt.ConnectionString));
                }
                else if (!string.IsNullOrWhiteSpace(opt.ConnectionName))
                {
                    EfCoreOptionsRegistrar.Register<TContext>(opt);
                    services.AddDbContext<TContext>((provider, builder) => builder.UseMySQL(provider.GetService<IConfigurationRoot>().GetConnectionString(opt.ConnectionName)));
                }
                else
                {
